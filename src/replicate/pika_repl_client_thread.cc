...
PikaReplWriterIOThread::PikaReplWriterIOThread(Transporter* transporter,
    int cron_interval, int keepalive_timeout)
  : ClientThread(&conn_factory_, cron_interval, keepalive_timeout, &handle_, NULL),
  transporter_(transporter) {
}

void PikaReplWriterIOThread::ReplWriterHandle::FdClosedHandle(int fd, const std::string& ip_port) const {
  ...
  transporter_->ReportUnreachable(ip, port, UnreachableReason::kFdClosed);
};

void PikaReplWriterIOThread::ReplWriterHandle::FdTimeoutHandle(int fd, const std::string& ip_port) const {
  ...
  //}
  //g_pika_server->UpdateMetaSyncTimestamp();
  transporter_->ReportUnreachable(ip, port, UnreachableReason::kFdTimeout);
};
