...
class ReplicationGroupNodeManager {
 public:
  ReplicationGroupNodeManager(ReplicationManager* rm) : replication_manager_(rm) { }
  ~ReplicationGroupNodeManager();

  static ReplicationGroupNodeManager& GetInstance() {
    static ReplicationGroupNodeManager instance;
    return instance;
  }

  Status CreateReplicationGroupNodes(const std::set<ReplicationGroupNodeConfig>& cfgs);
  ...
  void ReportUreachable(const NodeID& node_id, const UnreachableReason& reason);

  Status Propose(const ReplicationGroupID& copyset_id, const Task& task);

  // For compatibility
  // set the leader peer_id to the groups.
  void ActiveFollower(const PeerId& leader_id);
  Status UpdateLeader(const std::set<ReplicationGroupID>& ids,
                      const PeerID& leader_id,
                      bool force_full_sync);
  Status CreateReplicationGroupNodesSanityCheck(const std::set<ReplicationGroupID>& ids);
  Status RemoveReplicationGroupNodesSanityCheck(const std::set<ReplicationGroupID>& ids);
  Status SanityCheck(const SanityCheckFilter& fileter) {
    Status s;
    slash::RWLock l(rg_rw_, false);
    for (const auto& node : replication_groups_) {
      s = fileter(node);
      if (!s.ok()) {
        break;
      }
  ...
  Status DoSameThingEveryReplicationGroup(GroupEvent event);
  std::vector<ReplicationGroupNode> CurrentReplicationGroups();
 ...
 private:
  using ReplicationGroups = std::unordered_map<ReplicationGroupID,
                                               std::shared_ptr<ReplicationGroupNode>,
                                               hash_replication_group_id>;

  pthread_rwlock_t rg_rw_;
  ReplicationGroups replication_groups_;
...
class PikaReplicationManager {
 public:
  ...
  Status AddSyncPartitionSanityCheck(const std::set<ReplicationGroupID>& p_infos);
  Status AddSyncPartition(const std::set<ReplicationGroupID>& p_infos);
  Status RemoveSyncPartitionSanityCheck(const std::set<ReplicationGroupID>& p_infos);
  Status RemoveSyncPartition(const std::set<ReplicationGroupID>& p_infos);
  Status ActivateSyncSlavePartition(const RmNode& node, const ReplState& repl_state);
  Status DeactivateSyncSlavePartition(const ReplicationGroupID& p_info);
  ...
  std::shared_ptr<SyncMasterPartition> GetSyncMasterPartitionByName(const ReplicationGroupID& p_info);

  // For SyncSlavePartition
  std::shared_ptr<SyncSlavePartition> GetSyncSlavePartitionByName(const ReplicationGroupID& p_info);
 ...
 private:
  ...
  std::unordered_map<ReplicationGroupID, std::shared_ptr<SyncMasterPartition>, hash_partition_info> sync_master_partitions_;
  std::unordered_map<ReplicationGroupID, std::shared_ptr<SyncSlavePartition>, hash_partition_info> sync_slave_partitions_;
